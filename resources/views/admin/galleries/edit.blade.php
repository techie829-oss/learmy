...
<div class="max-w-3xl mx-auto bg-white rounded-[2.5rem] p-12 border border-gray-100 shadow-sm relative overflow-hidden">
    ...
    <div class="relative z-10">
        <div class="mb-12">
            <h3 class="text-3xl font-bold text-gray-900 mb-2">Edit Media Item</h3>
            <p class="text-gray-500 font-bold uppercase tracking-widest text-[10px]">Update the gallery item details</p>
        </div>
        ...
        <form action="{{ route('admin.galleries.update', $gallery->id) }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            @csrf @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div class="flex gap-4">
                    <label class="flex items-center gap-3 cursor-pointer p-4 rounded-2xl border-2 border-gray-200 hover:border-accent transition-colors flex-1 has-[:checked]:border-accent has-[:checked]:bg-accent/5">
                        <input type="radio" name="type" value="image" class="accent-yellow-600"
                            {{ old('type', $gallery->type) == 'image' ? 'checked' : '' }} onchange="toggleType(this.value)">
                        <span class="font-black text-sm text-gray-700">🖼️ Image Upload</span>
                    </label>
                    <label class="flex items-center gap-3 cursor-pointer p-4 rounded-2xl border-2 border-gray-200 hover:border-accent transition-colors flex-1 has-[:checked]:border-accent has-[:checked]:bg-accent/5">
                        <input type="radio" name="type" value="video" class="accent-yellow-600"
                            {{ old('type', $gallery->type) == 'video' ? 'checked' : '' }} onchange="toggleType(this.value)">
            ...
            {{-- Image Upload --}}
            <div id="imageField" class="space-y-2" style="{{ old('type', $gallery->type) == 'video' ? 'display:none' : '' }}">
                <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 block ml-2">Image <span class="text-gray-300">(Leave blank to keep current)</span></label>

                @if($gallery->file_path)
                    <div class="mb-4 rounded-2xl overflow-hidden border border-accent/20" style="max-height:220px">
                        <img src="{{ asset($gallery->file_path) }}" alt="Current image" class="w-full object-cover" style="max-height:220px">
                    </div>
                    <p class="text-xs text-gray-400 ml-2 mb-2">⬆️ Current image - upload a new one to replace it</p>
                @endif
                ...
                <div class="relative group cursor-pointer" onclick="document.getElementById('imgInput').click()">
                    ...
                    <div id="imgPreviewBox" class="w-full border-2 border-dashed border-gray-300 rounded-3xl overflow-hidden group-hover:border-accent transition-colors min-h-[160px] flex items-center justify-center bg-gray-50">
                        <div id="imgPlaceholder" class="text-center p-8">
                            ...
                            <p class="text-gray-400 font-bold text-sm">Click to upload new image</p>
                            <p class="text-gray-300 text-xs mt-1">Max 5MB — JPG, PNG, WEBP</p>
                        </div>
                        <img id="imgPreview" src="" alt="Preview" class="hidden w-full max-h-60 object-cover">
                    </div>
                </div>
            </div>

            {{-- Video URL --}}
            <div id="videoField" class="space-y-2" style="{{ old('type', $gallery->type) != 'video' ? 'display:none' : '' }}">
                <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 block ml-2">YouTube / Video Embed URL</label>
