...
<div class="max-w-4xl mx-auto bg-white rounded-[2.5rem] p-12 border border-gray-100 shadow-sm relative overflow-hidden">
    ...
    <div class="relative z-10">
        <div class="mb-12">
            <h3 class="text-3xl font-bold text-gray-900 mb-2">Module Enhancement</h3>
            <p class="text-gray-500 font-bold uppercase tracking-widest text-[10px]">Step: Update course details and content</p>
        </div>

        <form action="{{ route('admin.courses.update', $course->id) }}" method="POST" enctype="multipart/form-data" class="space-y-10">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div class="space-y-4">
                    ...
                    <input type="text" name="title" value="{{ old('title', $course->title) }}" required class="w-full bg-gray-50 border-0 border-b-2 border-gray-200 focus:border-accent p-4 text-gray-900 outline-none transition-colors rounded-2xl" placeholder="Professional Piano Mastery">
                </div>
            ...
            <div class="space-y-4">
                ...
                <textarea name="description" rows="5" required class="w-full bg-gray-50 border-0 border-b-2 border-gray-200 focus:border-accent p-6 text-gray-900 outline-none transition-colors rounded-3xl" placeholder="Describe the course objectives and journey...">{{ old('description', $course->description) }}</textarea>
            </div>

            <div class="space-y-4">
                <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 block ml-2">Key Features (One per line)</label>
                <textarea name="features" rows="4" class="w-full bg-gray-50 border-0 border-b-2 border-gray-200 focus:border-accent p-6 text-gray-900 outline-none transition-colors rounded-3xl font-mono text-sm" placeholder="Feature 1&#10;Feature 2&#10;Feature 3">{{ old('features', $course->features) }}</textarea>
            </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative group">
                        @if($course->image_path)
                            <div class="mb-4">
                                 <img src="{{ asset($course->image_path) }}" class="h-20 w-auto rounded-xl border border-gray-100 mb-4 group-hover:scale-105 transition-transform shadow-lg">
                            </div>
                        @endif
            ...
            <div class="space-y-10 pt-10 border-t border-gray-100">
                ...
                <div class="space-y-4">
                    ...
                    <input type="text" name="meta_title" value="{{ old('meta_title', $course->meta_title) }}" class="w-full bg-gray-50 border-0 border-b-2 border-gray-200 focus:border-accent p-4 text-gray-900 outline-none transition-colors rounded-2xl" placeholder="Best Piano Classes in... | Learmy Institute">
                </div>

                <div class="space-y-4">
                    <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 block ml-2">Meta Description</label>
                    <textarea name="meta_description" rows="3" class="w-full bg-gray-50 border-0 border-b-2 border-gray-200 focus:border-accent p-6 text-gray-900 outline-none transition-colors rounded-3xl" placeholder="Join our professional piano mastery course to learn from experts...">{{ old('meta_description', $course->meta_description) }}</textarea>
                </div>

                <div class="space-y-4">
                    <label class="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 block ml-2">Meta Keywords (Comma separated)</label>
                    <input type="text" name="meta_keywords" value="{{ old('meta_keywords', $course->meta_keywords) }}" class="w-full bg-gray-50 border-0 border-b-2 border-gray-200 focus:border-accent p-4 text-gray-900 outline-none transition-colors rounded-2xl" placeholder="piano, music, education, institute">
                </div>
            </div>

            <div class="flex gap-6 mt-16">
                <button type="submit" class="flex-grow gold-button text-primary font-black py-5 rounded-3xl uppercase tracking-widest text-xs shadow-2xl transition-all">Update Module</button>
