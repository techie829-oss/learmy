...
<head>
  ...
  <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=AW-17998082974"></script>
    <script>
    ...
    <script>
        ...
        fbq('init', '3930703010569242');
        fbq('track', 'PageView');
    </script>
    <noscript><img height="1" width="1" style="display:none"
            src="https://www.facebook.com/tr?id=3930703010569242&ev=PageView&noscript=1" /></noscript>
    <!-- End Meta Pixel Code -->
        ...
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        primary: {
                            DEFAULT: @if ($themeMode == 'dark')
                                '#1A1A1A'
                            @else
                                '#FFFFFF'
                            @endif ,
                            foreground: @if ($themeMode == 'dark')
                                '#FFFFFF'
                            @else
                                '#1A1A1A'
                            @endif ,
                        ...
                        accent: '#D4AF37',
                        accentLight: '#E5C45C',
                        surface: @if ($themeMode == 'dark')
                            '#000000'
                        @else
                            '#F9FAFB'
                        @endif ,
                        borderDefault: @if ($themeMode == 'dark')
                            'rgba(212, 175, 55, 0.2)'
                        @else
                            'rgba(212, 175, 55, 0.1)'
                        @endif ,
    ...
    <style>
        ...
        .glass-nav {
            ...
            box-shadow: @if ($themeMode == 'dark')
                0 4px 30px rgba(0, 0, 0, 0.5)
            @else
                0 4px 60px rgba(212, 175, 55, 0.05)
            @endif
            ;

            border-bottom: 1px solid @if ($themeMode == 'dark')
        ...
        .light-premium-card {
            ...
            border: 1px solid @if ($themeMode == 'dark')
                rgba(212, 175, 55, 0.1)
            @else
...
<body
    class="@if ($themeMode == 'dark') bg-primary text-gray-200 @else bg-white text-gray-900 @endif font-sans selection:bg-accent selection:text-primary">

    <!-- Navigation -->
    <nav class="fixed w-full z-50 glass-nav transition-all duration-300" id="main-nav">
        <!-- Scroll Progress Bar -->
        <div class="absolute bottom-0 left-0 h-[2px] bg-accent transition-all duration-100 ease-out"
        ...
        <div class="container flex justify-between items-center py-3 md:py-4">
            <a href="{{ route('home') }}"
                ...
                <div
                    class="relative overflow-hidden rounded-lg md:rounded-xl bg-white p-1 md:p-1.5 shadow-sm gold-border">
                    <img src="{{ asset('logo.jpeg') }}" alt="LEARMY" class="h-10 md:h-14 w-auto object-contain">
                </div>
            </a>

            <div
                class="hidden lg:flex items-center gap-4 xl:gap-8 @if ($themeMode == 'dark') text-gray-200 @else text-gray-800 @endif font-bold text-[10px] xl:text-xs uppercase tracking-[0.2em]">
                <a href="{{ route('home') }}"
                    class="nav-link-animated hover:text-accent transition-colors py-2 px-1">Home</a>
                <a href="{{ route('about') }}"
                    class="nav-link-animated hover:text-accent transition-colors py-2 px-1">About</a>
                <a href="{{ route('courses') }}"
                    class="nav-link-animated hover:text-accent transition-colors py-2 px-1">Courses</a>
                <a href="{{ route('faculty') }}"
                    class="nav-link-animated hover:text-accent transition-colors py-2 px-1">Faculty</a>
                <a href="{{ route('gallery') }}"
                    class="nav-link-animated hover:text-accent transition-colors py-2 px-1">Gallery</a>
                <a href="{{ route('events') }}"
                    class="nav-link-animated hover:text-accent transition-colors py-2 px-1">Events</a>
                <a href="{{ route('testimonials') }}"
                    class="nav-link-animated hover:text-accent transition-colors py-2 px-1">Testimonials</a>
                <a href="{{ route('contact') }}"
                    class="nav-link-animated hover:text-accent transition-colors py-2 px-1">Contact</a>
            </div>

            <div class="flex items-center gap-3 md:gap-6">
                <a href="{{ route('contact') }}"
                    class="gold-button font-black py-2 md:py-3 px-5 md:px-8 rounded-full text-[9px] md:text-[10px] uppercase tracking-[0.3em] shadow-lg">Enroll
                    Now</a>
    ...
    <!-- Footer -->
    <footer
        class="@if ($themeMode == 'dark') bg-black @else bg-white @endif pt-20 pb-10 border-t gold-border mt-20 relative overflow-hidden">
        @if ($themeMode != 'dark')
        ...
        <div class="container mx-auto px-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16" id="footer-grid">
                <div>
                    <div class="mb-8 group">
                        <img src="{{ asset('logo.jpeg') }}" alt="LEARMY"
                            class="h-16 md:h-24 w-auto object-contain transition-transform duration-500 group-hover:scale-105">
                        <div class="mt-6">
                            <div class="w-32 h-1 bg-accent rounded-full mb-3"></div>
                            <span
                                class="text-lg uppercase tracking-[0.4em] @if ($themeMode == 'dark') text-white @else text-gray-900 @endif font-black">Educoach
                                Institute</span>
                        </div>
                    </div>
                    <p
                        class="@if ($themeMode == 'dark') text-gray-400 @else text-gray-600 @endif mb-6 leading-relaxed">
                        Empowering talent through a perfect blend of musical artistry and academic excellence. Join us
                        to discover your potential.
                    </p>
                    <div class="flex gap-4">
                        <a href="https://www.facebook.com/people/Learmy-Institute-and-Performing-Arts/61581677499045/"
                            target="_blank"
                            class="w-10 h-10 rounded-full @if ($themeMode == 'dark') bg-gray-900 text-white @else bg-gray-200 text-gray-900 @endif border gold-border flex items-center justify-center hover:bg-accent hover:text-primary transition-all">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                <path
                                    d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" />
                            </svg>
                        </a>
                        <a href="https://www.instagram.com/learmy_music/" target="_blank"
                            class="w-10 h-10 rounded-full @if ($themeMode == 'dark') bg-gray-900 text-white @else bg-gray-200 text-gray-900 @endif border gold-border flex items-center justify-center hover:bg-accent hover:text-primary transition-all">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                <path
                ...
                <div>
                    <h4
                        class="text-lg font-bold mb-6 @if ($themeMode == 'dark') text-white @else text-gray-900 @endif">
                        Quick Links</h4>
                ...
                <div>
                    ...
                    <ul class="space-y-4 @if ($themeMode == 'dark') text-gray-400 @else text-gray-600 @endif">
                        <li>
                            <a href="https://www.google.com/maps?q=12.969560623168945,77.7711410522461&z=17&hl=en"
                                target="_blank"
                                class="flex items-start gap-3 group hover:text-accent transition-colors">
                                <svg class="w-6 h-6 text-accent shrink-0 group-hover:scale-110 transition-transform"
                                ...
                                <span class="text-sm">Shop No-1, Vasupradha Residency, Kaithola Main Road,
                                    Nagondanahalli, Bengaluru, 560066</span>
                            </a>
                        </li>
                        <li>
                            <a href="tel:+917483424001"
                                class="flex items-center gap-3 group hover:text-accent transition-colors">
                                <svg class="w-5 h-5 text-accent shrink-0 group-hover:scale-110 transition-transform"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <li>
                            <a href="mailto:learmymusic@gmail.com"
                                class="flex items-center gap-3 group hover:text-accent transition-colors">
                                <svg class="w-5 h-5 text-accent shrink-0 group-hover:scale-110 transition-transform"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div
                class="pt-8 border-t @if ($themeMode == 'dark') border-gray-900 @else border-gray-200 @endif text-center @if ($themeMode == 'dark') text-gray-500 @else text-gray-600 @endif flex flex-col md:flex-row justify-between items-center gap-4">
    ...
    <a href="https://wa.me/917483424001" target="_blank" id="whatsappGuitar"
        class="fixed bottom-8 right-8 z-50 group" title="Chat on WhatsApp">
        <!-- Pulse rings -->
        ...
        <div class="relative guitar-float">
            <svg width="70" height="70" viewBox="0 0 100 100" class="drop-shadow-2xl guitar-rock">
                ...
                <path d="M50 8 L50 30
                         ...
                         Z" fill="url(#guitarGrad)" stroke="#8B7217" stroke-width="1.5"
                    filter="url(#guitarGlow)" />

                <!-- Guitar neck -->
                <rect x="46" y="5" width="8" height="28" rx="2" fill="#8B6914" stroke="#6B5210"
                    stroke-width="0.5" />

                <!-- Guitar frets -->
                <line x1="46" y1="10" x2="54" y2="10" stroke="#D4AF37"
                    stroke-width="0.5" opacity="0.6" />
                <line x1="46" y1="15" x2="54" y2="15" stroke="#D4AF37"
                    stroke-width="0.5" opacity="0.6" />
                <line x1="46" y1="20" x2="54" y2="20" stroke="#D4AF37"
                    stroke-width="0.5" opacity="0.6" />
                <line x1="46" y1="25" x2="54" y2="25" stroke="#D4AF37"
                    stroke-width="0.5" opacity="0.6" />
                ...
                <circle cx="50" cy="62" r="12" fill="none" stroke="#D4AF37" stroke-width="0.5"
                    opacity="0.4" />

                <!-- WhatsApp icon inside sound hole -->
                <g transform="translate(41, 53) scale(0.38)">
                    <path
                        d="M24 4.557c-5.523 0-10.003 4.48-10.003 10.003 0 1.762.464 3.417 1.274 4.854L14 24l4.696-1.23A9.958 9.958 0 0024 24.563c5.523 0 10.003-4.48 10.003-10.003S29.523 4.557 24 4.557zm0 18.315a8.272 8.272 0 01-4.22-1.157l-.303-.18-3.138.823.837-3.06-.197-.314a8.27 8.27 0 01-1.267-4.424c0-4.583 3.73-8.312 8.312-8.312 4.583 0 8.312 3.73 8.312 8.312 0 4.583-3.73 8.312-8.312 8.312zm4.56-6.224c-.25-.125-1.478-.73-1.707-.813-.229-.083-.395-.125-.562.125s-.645.813-.79.98c-.146.167-.291.188-.541.063-.25-.125-1.055-.389-2.009-1.24-.743-.662-1.244-1.48-1.39-1.73-.146-.25-.016-.385.11-.51.112-.112.25-.291.374-.437.125-.146.167-.25.25-.417.083-.167.042-.312-.021-.437s-.562-1.355-.77-1.855c-.203-.488-.41-.422-.562-.43-.146-.006-.312-.007-.479-.007s-.437.063-.666.312c-.229.25-.875.855-.875 2.084s.896 2.418 1.021 2.584c.125.167 1.762 2.69 4.271 3.772.597.258 1.063.412 1.426.527.599.19 1.144.163 1.575.099.48-.072 1.478-.604 1.687-1.188.208-.583.208-1.083.146-1.188-.063-.104-.229-.167-.479-.291z"
                        fill="#25D366" />
                </g>

                <!-- Guitar strings -->
                <line x1="48" y1="30" x2="48" y2="88" stroke="#F5E0A3"
                    stroke-width="0.3" opacity="0.5" class="guitar-string" />
                <line x1="50" y1="30" x2="50" y2="88" stroke="#F5E0A3"
                    stroke-width="0.4" opacity="0.5" class="guitar-string" />
                <line x1="52" y1="30" x2="52" y2="88" stroke="#F5E0A3"
                    stroke-width="0.3" opacity="0.5" class="guitar-string" />
            ...
            <div
                class="absolute right-full mr-3 top-1/2 -translate-y-1/2 bg-white text-gray-900 px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                Chat with us! 🎸
    ...
    <div id="mobile-menu"
        class="fixed inset-0 lg:hidden @if ($themeMode == 'dark') bg-primary @else bg-white @endif z-[99999] overflow-y-auto transform transition-all duration-300 translate-x-full opacity-0">
        <div class="flex flex-col h-full">
            ...
            <div class="flex-grow py-8 px-6 flex flex-col space-y-2">
                ...
                @foreach ($navItems as $item)
                    <a href="{{ $item[1] }}"
                        class="group flex justify-between items-center py-4 px-4 rounded-xl @if ($themeMode == 'dark') hover:bg-white/5 @else hover:bg-black/5 @endif transition-all">
                        <span
                            class="text-2xl font-serif font-bold @if ($themeMode == 'dark') text-white @else text-gray-900 @endif group-hover:text-accent">{{ $item[0] }}</span>
            ...
            <div class="p-6 border-t gold-border">
                <a href="{{ route('contact') }}"
                    class="w-full gold-button font-black py-5 rounded-2xl text-center uppercase tracking-[0.2em] shadow-lg">Start
                    Your Journey</a>
            </div>
        </div>
    </div>
